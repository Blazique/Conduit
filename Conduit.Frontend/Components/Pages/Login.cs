...

{
    ...
    public override async ValueTask<LoginPageModel> Update(LoginPageModel model, LoginPageCommand command)
    {
        switch (command)
        {
    ...
    public override Node[] View(LoginPageModel model, Func<LoginPageCommand, Task> dispatch)
    =>
    [
        div([@class(["auth-page"])], [
...
public interface LoginPageCommand;
public record SetLoginEmail(string? Email) : LoginPageCommand;
public record SetPasswordForLogin(string? Password) : LoginPageCommand;
public record LoginUser(Validated<Credentials> validatedCredentials) : LoginPageCommand;